...
class TemporalBlock(tf.keras.Model):
	def __init__(self, dilation_rate, nb_filters, kernel_size, 
		...
		# block1
		self.conv1 = layers.Conv2D(filters=nb_filters, kernel_size=kernel_size,
								   dilation_rate=dilation_rate, padding=padding, 
								   kernel_initializer=init, input_shape = (None, 28,28))
		self.batch1 = layers.BatchNormalization(axis=-1)
		self.ac1 = layers.Activation('relu')
		self.drop1 = layers.Dropout(rate=dropout_rate)
		
		# block2
		self.conv2 = layers.Conv2D(filters=nb_filters, kernel_size=kernel_size,
								   dilation_rate=dilation_rate, padding=padding, 
...
def run(train_X, valid_X, test_X, train_Y, valid_Y, test_Y, epochs, batch_size,lr=0.01,momentum=0):
	model = TCN(output_size = 1,num_channels = [25]*6,kernel_size = 3,dropout = 0.2)
	...
	return predicted_labels, eval_mse
